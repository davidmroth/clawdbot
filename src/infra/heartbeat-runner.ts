...
import { isSessionBusy as isSessionFileBusy } from "../agents/session-write-lock.js";
...
export async function runHeartbeatOnce(opts: {
  ...
  // Skip heartbeat if the session file is already locked by another operation (e.g., user chat)
  // This prevents lock contention timeouts when a long-running LLM call holds the lock.
  const sessionFilePath = resolveSessionFilePath(
    entry?.sessionId ?? sessionKey,
    entry,
    { agentId },
  );
  if (isSessionFileBusy(sessionFilePath)) {
    return { status: "skipped", reason: "session-in-use" };
  }
