...
const MEDIA_TAG_RE = /\[\[MEDIA:[^\]]*\]\]/gi;

// Combined pattern for simple stripping (no metadata extraction)
const ALL_DIRECTIVE_TAGS_RE =
  /\[\[(?:reply_to|reply_to_current|audio_as_voice|MEDIA)[^\]]*\]\]/gi;

/**
 * Simple tag stripping without metadata extraction.
 * Use this when you only need clean text (e.g., UI display).
 */
export function stripDirectiveTags(text: string): string {
  if (!text || !text.includes("[[")) return text ?? "";
  return text
    .replace(ALL_DIRECTIVE_TAGS_RE, " ")
    .replace(/[ \t]+/g, " ")
    .replace(/[ \t]*\n[ \t]*/g, "\n")
    .trim();
}
