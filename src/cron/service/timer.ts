    ...
    if (job.sessionTarget === "isolated") {
      ...
      if (mode === "full") {
        ...
        const maxChars = Number.isFinite(maxCharsRaw)
          ? Math.max(0, maxCharsRaw as number)
          : 8000;
      ...
      const fullMessage = `${statusPrefix}: ${body}`;

      // Broadcast to UI as a separate assistant message (not prepended to user input)
      if (state.deps.broadcastChatMessage) {
        // Use main session key for UI display
        state.deps.broadcastChatMessage("agent:main:main", fullMessage);
      }

      // Still enqueue for agent context (but this won't pollute user messages in UI)
      state.deps.enqueueSystemEvent(fullMessage, {
        agentId: job.agentId,
      });
      if (job.wakeMode === "now") {
