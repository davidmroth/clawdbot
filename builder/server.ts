...
const runCommand = (command: string, args: string[], logFn: (msg: string) => void): Promise<void> => {
        ...
        try {
            ...
            if (existsSync(DIST_DIR)) {
                ...
                try {
                    ...
                    log(`[Builder] Rename failed (cross-device?), copying instead: ${e}\n`);
...
server.listen(PORT, () => {
    ...
    console.log(`[Builder] Trigger with: curl -N -X POST http://localhost:${PORT}/build`);
});
